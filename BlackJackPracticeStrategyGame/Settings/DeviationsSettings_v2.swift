...
class DeviationsSettings_v2: GameTypeSettings {
    var title: String = "Deviations #2"
...
//                    action: { _ in }),
            ...
            Section(title: "Hand Types", rows: [
                ...
                SwitchRow(text: "Hard Totals", switchValue: settings.hardHands, customization: {cell,row in
                    ...
                    //if self.settings.hardHands == false {
                        // disable 3/4 card hands
                        self.setupThreeCardHandsCell()
                    self.setupFourCardHandsCell()
                    //}
...
//                        (self.quickFeedbackCell.accessoryView as! UISwitch).sendActions(for: .valueChanged)
    ...
    private func setupThreeCardHandsCell() {
        if settings.hardHands {
            threeHandCell.isUserInteractionEnabled = true
            threeHandCell.textLabel!.isEnabled = true
            (threeHandCell.accessoryView as! UISwitch).isEnabled = true
            //(threeHandCell.accessoryView as! UISwitch).setOn(settings.splitHands, animated: true)
            //(self.splitCell.accessoryView as! UISwitch).sendActions(for: .valueChanged)
        } else {
            threeHandCell.isUserInteractionEnabled = false
            threeHandCell.textLabel!.isEnabled = false
            (threeHandCell.accessoryView as! UISwitch).isEnabled = false
            (threeHandCell.accessoryView as! UISwitch).setOn(false, animated: true)
            (self.threeHandCell.accessoryView as! UISwitch).sendActions(for: .valueChanged)
        }
    }
    private func setupFourCardHandsCell() {
        if settings.hardHands {
            fourHandCell.isUserInteractionEnabled = true
            fourHandCell.textLabel!.isEnabled = true
            (fourHandCell.accessoryView as! UISwitch).isEnabled = true
            //(threeHandCell.accessoryView as! UISwitch).setOn(settings.splitHands, animated: true)
            //(self.splitCell.accessoryView as! UISwitch).sendActions(for: .valueChanged)
        } else {
            fourHandCell.isUserInteractionEnabled = false
            fourHandCell.textLabel!.isEnabled = false
            (fourHandCell.accessoryView as! UISwitch).isEnabled = false
            (fourHandCell.accessoryView as! UISwitch).setOn(false, animated: true)
            (self.fourHandCell.accessoryView as! UISwitch).sendActions(for: .valueChanged)
        }
    }
