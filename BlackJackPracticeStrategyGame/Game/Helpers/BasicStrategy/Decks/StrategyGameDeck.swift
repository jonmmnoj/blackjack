...
//    }
    ...
    static var twoCardHandDeck: StrategyDeck {
        ...
        for i in 0...twoCardHands.count - 1 {
            ...
            for j in 0...values.count - 1{
                let sa = BasicStrategy.getPlayerAction(dealerCardValue: values[j], playerCardValues: [hand[0], hand[1]])
                let pc = [ hand[0], hand[1]]
                let dc = [values[j]]
                
                if !StrategyGameDeck.keepHandForStrategyDeck(playerCardValues: pc, dealerCardValue: dc[0]) {
                    continue
                }
                let SR = StrategyRound(playerCards: pc, dealerCards: dc, action: sa.action)
    ...
    static var threeCardHandDeck: StrategyDeck {
        ...
        for hand in threeCardHands {
            ...
            let dc = [hand[3]]
            let sa = BasicStrategy.getPlayerAction(dealerCardValue: dc[0], playerCardValues: pc)
            let SR = StrategyRound(playerCards: pc, dealerCards: dc, action: sa.action)
            deck.rounds.append(SR)
        }
        //print(deck.rounds.count)
...
//                        }
//                        let dc = [ values[j]]
//                        if !StrategyGameDeck.keepHandForStrategyDeck(playerCardValues: [hand[0], hand[1], values[k]], dealerCardValue: dc[0]) {
//                            continue
//                        }
//
...
//        }
    ...
    static private func keepHardHand(playerRuleValue: Int, dealerCardValue: Int) -> Bool {
        switch playerRuleValue {
        case 17:
            if (dealerCardValue == 1 || dealerCardValue >= 9 && dealerCardValue <= 10) && Settings.shared.surrender && Settings.shared.dealerHitsSoft17 {
                return true
            } else {
                return false
            }
        case 16:
            return true//dealerCardValue == 1 || dealerCardValue >= 2 && dealerCardValue <= 10
        ...
        case 14:
            return dealerCardValue >= 5 && dealerCardValue <= 7
        case 13:
            return dealerCardValue == 2 || (dealerCardValue >= 5 && dealerCardValue <= 7)
        case 12:
            return dealerCardValue >= 2 && dealerCardValue <= 7
        case 11:
            return dealerCardValue == 1 || dealerCardValue >= 10 && dealerCardValue <= 10
        case 10:
            return dealerCardValue == 1 || dealerCardValue >= 8 && dealerCardValue <= 10
        case 9:
            return dealerCardValue >= 2 && dealerCardValue <= 8
        case 8:
            return dealerCardValue >= 6 && dealerCardValue <= 6
        default:
            return false
        }
    }
