...
class RunningCountV2GameHelper: FreePlayGameTypeStrategy {
    ...
    private func getDealPoint(for stackNumberIndex: Int) -> CGPoint {
        //var dealPoint = CGPoint(x: (UIScreen.main.bounds.midX * 0.5) - Card.width * 0.5, y: UIScreen.main.bounds.midY - Card.height * 0.5)
        var dealPoint = CGPoint(x: UIScreen.main.bounds.midX - Card.width * 1.5, y: UIScreen.main.bounds.midY - Card.height * 0.5)
        if Settings.shared.verticalSizeClass == .compact {
            dealPoint = CGPoint(x: (UIScreen.main.bounds.midX * 0.5) - Card.width * 0.5, y: UIScreen.main.bounds.midY - Card.height * 0.5)
        }
        
        if Settings.shared.rcNumberOfStacks == 1 {
